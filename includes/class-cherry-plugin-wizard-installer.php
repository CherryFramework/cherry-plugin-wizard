...
if ( ! class_exists( 'Cherry_Plugin_Wizard_Installer' ) ) {

	/**
	 * Define Cherry_Plugin_Wizard_Installer class
	 */
	class Cherry_Plugin_Wizard_Installer {
		...
		function __construct() {
			add_action( 'wp_ajax_cherry_plugin_wizard_install_plugin', array( $this, 'install_plugin' ) );
			add_action( 'wp_ajax_cherry_plugin_wizard_process_single_plugin', array( $this, 'process_single_plugin' ) );
		}
		...
		public function install_plugin() {
			...
			if ( ! current_user_can( 'install_plugins' ) ) {
				wp_send_json_error(
					array( 'message' => esc_html__( 'You don\'t have permissions to do this', 'cherry-plugin-wizard' ) )
				);
			}
			...
			if ( ! $plugin || ! $skin || ! $type ) {
				wp_send_json_error(
					array( 'message' => esc_html__( 'No plugin to install', 'cherry-plugin-wizard' ) )
				);
			}

			$this->do_plugin_install( cherry_plugin_wizard_data()->get_plugin_data( $plugin ) );

			$next = cherry_plugin_wizard_data()->get_next_skin_plugin( $plugin, $skin, $type );
			...
			if ( $first ) {
				...
				if ( $active_skin ) {
					add_filter( 'cherry_plugin_wizard_deactivate_skin_plugins', array( $this, 'unset_next_skin_plugins' ) );
					$this->deactivate_skin_plugins( $active_skin['skin'], $active_skin['type'] );
					remove_filter( 'cherry_plugin_wizard_deactivate_skin_plugins', array( $this, 'unset_next_skin_plugins' ) );
				}
			}

			if ( ! $next ) {

				$message  = esc_html__( 'All plugins are installed. Redirecting to the next step...', 'cherry-plugin-wizard' );
				$redirect = apply_filters(
					'cherry_plugin_wizards_install_finish_redirect',
					cherry_plugin_wizard()->get_page_link( array( 'step' => 4, 'skin' => $skin, 'type' => $type ) )
				);

				delete_option( 'cherry_plugin_wizard_show_notice' );
				delete_option( 'tm_active_skin' );
				delete_option( cherry_plugin_wizard_data()->advances_plugins );
				add_option( 'tm_active_skin', array( 'skin' => $skin, 'type' => $type ), '', false );

				do_action( 'cherry_plugin_wizard_install_finished' );

				$data = array(
					'isLast'     => true,
					'message'    => sprintf( '<div class="cherry-plugin-wizard-installed">%s</div>', $message ),
					'redirect'   => $redirect,
					'log'        => $this->log,
		...
		public function send_success( $data = array(), $plugin = '' ) {
			...
			wp_send_json_success( apply_filters( 'cherry_plugin_wizard_send_install_data', $data, $plugin ) );
		...
		public function unset_next_skin_plugins( $plugins = array() ) {
			...
			$skin_plugins = cherry_plugin_wizard_data()->get_skin_plugins( $skin );
		...
		public function deactivate_skin_plugins( $skin = null, $type = null ) {
			...
			$skins   = cherry_plugin_wizard_settings()->get( array( 'skins' ) );
			$plugins = isset( $skins['advanced'][ $skin ][ $type ] ) ? $skins['advanced'][ $skin ][ $type ] : array();
			$active  = get_option( 'active_plugins' );
			$plugins = apply_filters( 'cherry_plugin_wizard_deactivate_skin_plugins', $plugins, $kin );
		...
		public function process_single_plugin() {
			...
			wp_send_json_error( array(
				'message' => esc_html__( 'Action not provided', 'cherry-plugin-wizard' ),
			) );
		...
		public function install_single_plugin() {
			...
			if ( ! $slug ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'Plugin slug not provided', 'cherry-plugin-wizard' ),
				) );
			}

			$this->do_plugin_install( cherry_plugin_wizard_data()->get_plugin_data( $slug ), false );
			...
			if ( 'success' === $result_type ) {
				...
				ob_start();
				cherry_plugin_wizard_ext()->single_plugin_item( $slug, cherry_plugin_wizard_data()->get_plugin_data( $slug ) );
				$item = ob_get_clean();
				...
				wp_send_json_error( array(
					'message' => esc_html__( 'Installation failed', 'cherry-plugin-wizard' ),
					'log'     => $this->log
				) );
			}
		...
		public function activate_single_plugin() {
			...
			if ( ! $path || ! $slug ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'Plugin data not provided', 'cherry-plugin-wizard' ),
				) );
			}
			...
			if ( ! is_wp_error( $activate ) ) {
				...
				wp_send_json_error( array(
					'message' => esc_html__( 'Can\'t perform plugin activation. Please try again later', 'cherry-plugin-wizard' ),
				) );
			}
		...
		public function do_plugin_install( $plugin = array(), $activate = true ) {
			 ...
			 */
			do_action( 'cherry_plugin_wizard_before_plugin_install', $plugin );
			...
			$this->installer = new Cherry_Plugin_Wizard_Plugin_Upgrader(
				new Cherry_Plugin_Wizard_Plugin_Upgrader_Skin(
					array(
						'url'    => false,
						'plugin' => $plugin['slug'],
			 ...
			 * @param array $plugin Plugin data array.
			 */
			do_action( 'cherry_plugin_wizard_after_plugin_install', $plugin );
			 ...
			 * @param array $plugin Plugin data array.
			 */
			do_action( 'cherry_plugin_wizard_after_plugin_activation', $plugin );
		...
		public function dependencies() {
			...
			require_once cherry_plugin_wizard()->path( 'includes/class-cherry-plugin-wizard-plugin-upgrader-skin.php' );
			require_once cherry_plugin_wizard()->path( 'includes/class-cherry-plugin-wizard-plugin-upgrader.php' );
		}
...
/**
 * Returns instance of Cherry_Plugin_Wizard_Installer
 *
 * @return object
 */
function cherry_plugin_wizard_installer() {
	return Cherry_Plugin_Wizard_Installer::get_instance();
}

cherry_plugin_wizard_installer();
