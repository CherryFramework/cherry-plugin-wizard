...
if ( ! class_exists( 'Cherry_Plugin_Wizard_Data' ) ) {

	/**
	 * Define Cherry_Plugin_Wizard_Data class
	 */
	class Cherry_Plugin_Wizard_Data {
		...
		public function __construct() {
			add_action( 'wp_ajax_cherry_plugin_wizard_store_plugins', array( $this, 'store_plugins' ) );
		}
		...
		public function store_plugins() {
			...
			if ( empty( $_REQUEST['plugins'] ) ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'Plugins array are empty', 'cherry-plugin-wizard' )
				) );
			}
		...
		public function default_skin() {
			...
			$skins      = cherry_plugin_wizard_settings()->get( array( 'skins', 'advanced' ) );
		...
		public function sanitize_type( $type ) {
			$allowed = apply_filters( 'cherry_plugin_wizard_allowed_install_types', array( 'lite', 'full' ) );
		...
		public function get_first_plugin_data( $skin = null, $type = null ) {
			...
			$plugins         = cherry_plugin_wizard_data()->get_first_skin_plugin();
		...
		public function is_single_type_skin( $skin = '' ) {
			$skin_data = cherry_plugin_wizard_settings()->get( array( 'skins', 'advanced', $skin ) );
...
/**
 * Returns instance of Cherry_Plugin_Wizard_Data
 *
 * @return object
 */
function cherry_plugin_wizard_data() {
	return Cherry_Plugin_Wizard_Data::get_instance();
}

cherry_plugin_wizard_data();
